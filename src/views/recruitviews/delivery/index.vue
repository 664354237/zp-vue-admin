        ...
        <div class="home-content">
            <table-content :AllTitle='AllTitle' :tableData='tableData' :table='table'>
                    ...
                    <div class="search-flex-item">
                        <div class="search-item">
                            ...
                            <el-select v-model="value" placeholder="请选择">
                                <el-option v-for="item in selectSex" :key="item.value" :label="item.label" :value="item.value"></el-option>
                            </el-select>
                        </div>
                        <div class="search-item">
                ...
                <template slot="status" slot-scope="{row}">
                    <el-button icon="el-icon-user" class="btnblue" size="mini" type="primary" @click="handleDiagram(row)">查看进度</el-button>
                </template>
            </table-content>
        </div>
    </div>
...
const COLUMNS = [
  ...
  { prop: 'dataType', label: '学院/单位' },
  { prop: 'id', label: '投递时间' },
  { prop: 'id', label: '状态' },
  { prop: 'status', label: '操作', enableSlot: true, width: 180 }
]
export default {
    name: 'delivery',
    data() {
